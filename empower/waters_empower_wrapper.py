...
class WatersEmpower:
    ...
    def __init__(self, exe_directory: str = None):
        ...
        self.sample_set_extractor = os.path.join(
            exe_directory, "SampleSetExtractor.exe"
        )
        ...
        try:
            ...
            if result["success"]:
                ...
                for line in lines:
                    ...
                    if (
                        line
                        and not line.startswith("Waters")
                        and not line.startswith("=")
                        and not line.startswith("Loading")
                        and not line.startswith("✅")
                        and not line.startswith("🎉")
                        and not line.startswith("Creating")
                        and not line.startswith("Attempting")
                        and "sample set methods" not in line.lower()
                    ):
    ...
    def create_sample_set(
        ...
        try:
            result = self._run_executable(
                self.sample_set_creator, [sample_set_name], timeout=60
            )

            if (
                result["success"]
                and "NEW SAMPLE SET CREATED SUCCESSFULLY" in result["output"]
            ):
                # Extract details from output
                details = self._parse_sample_set_details(
                    result["output"], sample_set_name
                )
    ...
    def run_sample_set(
        ...
        try:
            ...
            result = self._run_executable(self.sample_set_extractor, args, timeout=120)
    ...
    def get_status(self) -> Dict[str, Union[bool, str]]:
        ...
        try:
            ...
            result = self._run_executable(
                self.sample_set_extractor, ["--status"], timeout=30
            )
    ...
    def _run_executable(
        self, exe_path: str, args: List[str], timeout: int = 60
    ) -> Dict[str, Union[bool, str]]:
        """
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=timeout, cwd=self.exe_dir
            )
    ...
    def _parse_sample_set_details(
        self, output: str, sample_set_name: str
    ) -> Dict[str, str]:
